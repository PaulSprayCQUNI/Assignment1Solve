...
public class FSAGUI extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        comboboxRentalType = new JComboBox<>();
        ...
        comboboxNumberOfRooms = new JComboBox<>();
        labelNumberOfBathrooms = new JLabel();
        comboboxNumberOfBathrooms = new JComboBox<>();
    ...
    private void btnViewRentalsActionPerformed(ActionEvent evt) {//GEN-FIRST:event_btnViewRentalsActionPerformed
        ...
        new ViewRentalsGUI(currentRental);
    ...
    private void btnEditRentalActionPerformed(ActionEvent evt) {//GEN-FIRST:event_btnEditRentalActionPerformed
        ...
        tfRentalID.setText(currentRental.getRentalID());
        tfAddress.setText(currentRental.getAddress());
        tfWeeklyPrice.setText(String.format("%.2f", currentRental.getWeeklyPrice()));
        cbFurnished.setSelected(currentRental.getFurnished());
        tfDescription.setText(currentRental.getDescription());        
        ...
        if (currentRental instanceof RoomRental) 
        {
            ...
            cbCouplesAllowed.setSelected(((RoomRental)currentRental).getCouplesAllowed());
            cbAttachedBathroom.setSelected(((RoomRental)currentRental).getAttachedBathroom());
        }
        ...
        if (currentRental instanceof WholeRental) 
        {
            ...
            comboboxNumberOfRooms.setSelectedIndex(((WholeRental)currentRental).getNumberOfRooms());
            comboboxNumberOfBathrooms.setSelectedIndex(((WholeRental)currentRental).getNumberOfBathrooms());
            cbGarageSpace.setSelected(((WholeRental)currentRental).getGarageSpace());
            cbPetsAllowed.setSelected(((WholeRental)currentRental).getPetsAllowed());
        }
        ...
        btnBack.setEnabled(false);   // disabled for now since there is no more than 1 Rental object
    ...
    private void btnEditActionPerformed(ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        ...
        btnUpdate.setEnabled(true); // [Update] button enabled to save changes
    ...
    private void btnUpdateActionPerformed(ActionEvent evt) {//GEN-FIRST:event_btnUpdateActionPerformed
        ...
        currentRental.setAddress(tfAddress.getText());
        currentRental.setWeeklyPrice(Double.parseDouble(tfWeeklyPrice.getText()));
        currentRental.setFurnished(cbFurnished.isSelected());
        currentRental.setDescription(tfDescription.getText());

        if (currentRental instanceof RoomRental) 
        {
            RoomRental current = (RoomRental)currentRental;
        ...
        if (currentRental instanceof WholeRental) 
        {
            WholeRental current = (WholeRental)currentRental;
        ...
        btnBack.setEnabled(false);   // disabled for now since there is no more than 1 Rental object   
        btnNext.setEnabled(false);   // disabled for now since there is no more than 1 Rental object
        btnEdit.setEnabled(true);   // [Edit] button reenabled once update is complete
    ...
    private Rental currentRental;       // used to store most recently added Rental object
                                        // TO UPDATE: create an ArrayList or LinkedList to store all Rental objects
